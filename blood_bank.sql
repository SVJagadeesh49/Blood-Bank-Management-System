...
CREATE TABLE `bb_blood` (
  ...
  constraint primary key(blood_id)
...
CREATE TABLE `bb_hospital` (
  ...
  `status` tinyint(4) NOT NULL,
  constraint primary key(hospital_id)
...
CREATE TABLE `bb_user` (
  ...
  constraint foreign key(blood_id) references bb_blood(blood_id)
...
CREATE TABLE `bb_request` (
  ...
  `datetime` datetime NOT NULL,
  `status` tinyint(4) NOT NULL,
  constraint primary key(request_id),
  constraint foreign key(user_id) references bb_user(user_id),
  constraint foreign key(hospital_id) references bb_hosiptal(hospital_id),
  constraint foreign key(blood_id) references bb_blood(blood_id)
